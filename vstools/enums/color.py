...
from stgpytools import CustomEnum, FuncExceptT, classproperty

from ..exceptions import (
    NotFoundEnumValue, ReservedMatrixError, ReservedPrimariesError, ReservedTransferError, UndefinedMatrixError,
    UndefinedPrimariesError, UndefinedTransferError, UnsupportedColorRangeError, UnsupportedMatrixError,
...
class RGBColor(list[float], CustomEnum):
    ...
    def to_clip(self, ref: vs.VideoNode | None = None) -> vs.VideoNode:
        ...
        return blank_clip.resize.Bicubic(format=ref.format, matrix=matrix)
    ...
    def scale_value(
        ...
        from ..utils.scale import scale_value

        # TODO: idk what to do with the colour range
        return [scale_value(value, 32, bitdepth, range_in=ColorRange.FULL) for value in self.value]
